...
function ModalDetailsComponent({ isOpen, onClose, trackingDetails }) {
  const scannerContext = useScanner();

  if (!isOpen) return null;

  const handleDelete = () => {
    if (trackingDetails?.id) {
      scannerContext.deleteTracking(trackingDetails.id);
      onClose();
    }
  };
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <p className="font-medium">{trackingDetails?.deliveryDate}</p>
            </div>
            <div>
              <p className="text-sm text-gray-600">Location</p>
              <p className="font-medium">{trackingDetails?.facility}</p>
            </div>
          </div>
